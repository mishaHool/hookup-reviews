function MemeberViewControl() { var e, s, o = $(".set-view-more-btn .members-block"),
        a = $(".set-view-more-btn .member"),
        i = $(".set-view-more-btn").data("start"),
        r = $(window).width(),
        t = $(".cssload-container"),
        l = $(".member_btn i"); for (e = 0; e < i; ++e) a.eq(e).addClass("show"); var n = $(".set-view-more-btn").data("add-desktop"),
        c = $(".set-view-more-btn").data("add-tablet"),
        d = $(".set-view-more-btn").data("add-mobile");
    s = 1, $(".set-view-more-btn .member_btn button").click(function() { if (t.addClass("vis"), l.addClass("invis"), o.css({ opacity: .5 }), s > 2) return show_reg_form("centered"), t.removeClass("vis"), l.removeClass("invis"), o.css({ opacity: 1 }), !1;
        setTimeout(function() { if (r >= 770) { for (e = i; e < i + n * s; ++e) a.eq(e).addClass("show");
                t.removeClass("vis"), l.removeClass("invis"), o.css({ opacity: 1 }), ++s } if (r < 770 && r >= 590) { for (e = i; e < i + c * s; ++e) a.eq(e).addClass("show");
                t.removeClass("vis"), l.removeClass("invis"), o.css({ opacity: 1 }), ++s } if (r < 590) { for (e = i; e < i + d * s; ++e) a.eq(e).addClass("show");
                t.removeClass("vis"), l.removeClass("invis"), o.css({ opacity: 1 }), ++s } }, 1e3);
...
$(function() { MemeberViewControl(), $("div.members-block div.member div.img, div.members-block div.member div.username p, .member .actions a, div.members-block .member-info").click(function() { var e = $("div.form-Registration"),
            s = $(this).parents(".member"),
            o = $(".member_in_form"); if (e.hasClass("set-form-with-member")) { e.addClass("show"); var a = s.html();
            $(".member_in_form").html('<div class="close">&times;</div>'), $(".member_in_form .close").after(a); var i = s.hasClass("online") ? "online" : s.hasClass("offline") ? "offline" : "none";
            o.removeClass("online").removeClass("offline").addClass(i), $(".member_in_form img").click(function() { $(window).width();
